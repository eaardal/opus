...
| **Magnifier** | A read-only way to enlarge a task for closer reading: hovering a task while holding Alt/Option (or with the magnifier toolbar toggle on) shows an enlarged copy of it floating over the canvas. If the task belongs to a group, the group's title is shown above the enlarged task; hovering a group itself shows just the group's title. It never changes anything — it's purely a viewing aid. |
